...
import { model_cart } from '../Cart/model';
...
export const get = async (req:Request, res:Response, next:NextFunction) => {
    try {
        ...
        if (_id) {
            const product = model_cart.findById(_id);
        ...
        if (Object.keys(query).length > 0) {
            const products = await model_cart.find().where(query).limit(Number(_quantity)||0);
            return res.status(200).json(products);
        };

        const allProducts = await model_cart.find().limit(Number(_quantity)||0);
...
export const register = async (req:Request, res:Response, next:NextFunction) => {
    ...
    try {
        ...
        const product = await model_cart.create({...req.body, user});
